...
def load_im(exp): 
    ...
    sar_root_path = '../../../datasets/LEM/Sentinel_1/'
    opt_root_path = '../../../datasets/LEM/Landsat_8/Images/'
    lbl_path = '../../../datasets/LEM/Landsat_8/Reference/'

    sar_img_name = '01_20170612_S1A_10m'
    opt_img_name = '02_20170615_L08_30m'
    lbl_name = '03_Jun2017_10m'

    # Mask train test
    mask_train_test = np.load('../../../datasets/LEM/Masks/mask_train_test.npy').astype('float32')
    mask_nan = np.load('../../../datasets/LEM/Masks/mask_nan.npy').astype('float32')
    mask_train_test[mask_nan == 1] = 0.3
    rows, cols = mask_train_test.shape
    save_mask = Image.fromarray(np.uint8(mask_train_test*255))
    save_mask.save('mask_train_test.tif')

    # Labels
    labels = np.array(Image.open(lbl_path + lbl_name + '.tif'))
    
    if exp == 1:
        # Original
        I = load_landsat(opt_root_path + opt_img_name)
        I = Normalization(I)
        # I = np.concatenate((I, NDVI_band(I)), axis=2)
        cols //= 3; rows //= 3
        mask_train_test = np.array(Image.fromarray(mask_train_test).resize((cols, rows), Image.NEAREST))
        labels = np.array(Image.fromarray(labels).resize((cols, rows), Image.NEAREST))

    elif exp == 2:
        # Fake atrous
        fake_name = '../../test_images/LEM/atrous/atrous_bs2_min_max_ps256_op92_ep50_LessLrDisc.npz'
        ...
        # Fake pix2pix
        fake_name = '../../test_images/LEM/pix2pix/pix2pix_bs2_min_max_ps256_op92_ep50.npz'
        ...
        # SAR
        I = np.load(sar_root_path + sar_img_name + '/' + sar_img_name + '.npy').astype('float32')
        I = Normalization(I)
    
    print('Percent of all classes:')
    val, counts = np.unique(labels, return_counts=True)
    val = val[np.argsort(counts)[::-1]]
    counts = np.sort(counts)[::-1]
    print(val, np.around(100*counts / np.sum(counts), decimals=1))

    print('Percent of positive classes:')
    val, counts = np.unique(labels, return_counts=True)
    val = val[np.argsort(counts)[::-1]]
    counts = np.sort(counts)[::-1]
    print(val[1:], np.around(100*counts[1:] / np.sum(counts[1:]), decimals=1))

    # Merging minority classes
    for i in val:
        if (i != 0) and (i != 13) and (i != 7) and (i != 11):
            labels[labels == i] = np.max(val) + 1
    ...
    return I, labels, mask_train_test
...
if __name__=='__main__':
    ...
    for exp in Exp:
        for Arq in arq:
            ...
            N_Run = 5
            overlap_percent =  0.80
            if exp == 4: overlap_percent =  0.50
            ...
            fold_dir = './LEM results____experiment(%d)' %(exp)
            ...
            print(val[1:], np.around(perc_labels, decimals=1))

            # Classes weights
            weights = scale - (scale * perc_labels / 100)
            weights = np.concatenate(([0.0], weights), axis=0)
            print('labels:  ', val)
            print('weights: ', np.round(weights, decimals=1))
...
#%% 
                ...
                file_metrics.write('Clases:     '); file_metrics.write(str(np.unique(y_pred)));      file_metrics.write('\n')
