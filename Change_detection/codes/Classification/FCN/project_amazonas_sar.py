...
def load_im(): 
    ...
    dir = '../../../datasets/Amazonia_Legal/Sentinel_1/'
    Images_past_file = dir +  '2018_07/2018_07.npy'
    Image_present_file = dir + '2019_07/2019_07.npy'
    
    dir = '../../../datasets/Amazonia_Legal/Landsat_8/Reference/'
    past_reference_file = dir + 'PAST_REFERENCE_FOR_2019_EPSG32620.npy'
    reference_file = dir + "REFERENCE_2019_EPSG32620.npy"
    
    d0 = np.load(Images_past_file)
    d1 = np.load(Image_present_file)

    I = np.concatenate((d0, d1), axis = 2) 

    past_reference = np.load(past_reference_file)
    act_reference = np.load(reference_file)

    past_reference = resize(past_reference, (I.shape[0], I.shape[1]), preserve_range=True, order=0)
    act_reference = resize(act_reference, (I.shape[0], I.shape[1]), preserve_range=True, order=0)

    I = I[3*1:3*2551, 3*1:3*5121, :]
    past_reference = past_reference[3*1:3*2551, 3*1:3*5121]
    act_reference = act_reference[3*1:3*2551, 3*1:3*5121]

    return I, past_reference, act_reference
...
#    from Arquitecturas.fusion import Tiramisu
    ...
    if Arq == 1:
        print("Start Segnet")
        net = Segnet(nClasses = len(np.unique(reference)), optimizer = None, input_width = patch_size , input_height = patch_size , nChannels = chanels)

    elif Arq == 2:
        print("Start Unet")
        net = Unet(len(np.unique(reference)), patch_size, patch_size , chanels) 

    elif Arq ==3:
        print("Start DeepLabv3p")
        net = Deeplabv3p(input_tensor=None, infer = False,
                input_shape=(patch_size, patch_size, chanels), classes= len(np.unique(reference)), backbone='mobilenetv2', OS=16, alpha=1.)
    else:           
        print("Start DenseNet")
        net = Tiramisu(input_shape = (patch_size, patch_size, chanels), n_classes = len(np.unique(reference)), n_filters_first_conv = 32, 
...
if __name__=='__main__':
    ...
    for kk in range(1, 5):
        fold_dir = './Amazonia_Legal_SAR_fold_' + str(kk)
        if not os.path.exists(fold_dir):
            os.makedirs(fold_dir)

        I, past_reference, act_reference = load_im()
        rows, cols, chanels = I.shape
        Norm_image = Normalization(I) 

        mask = Image.open('../../../datasets/Amazonia_Legal/mask_train_val_test_fold_' + str(kk) + '.tif')
        mask = np.array(mask) / 255
        mask = resize(mask, (I.shape[0], I.shape[1]), preserve_range=True, order=0)
