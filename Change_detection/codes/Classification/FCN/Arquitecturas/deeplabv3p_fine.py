...
def Deeplabv3p(weights='pascal_voc', input_tensor=None, infer = False,
              input_shape=(512, 512, 3), classes=21, backbone='mobilenetv2',
              OS=16, alpha=1.,include_top = True,skip1 = None):
    ...
    """
#    If include_top is false I am going to include only the encoder 
#    if is true I am going to include only the encoder
    ...
    if include_top:
        if backbone == 'mobilenetv2':
            OS = 8
        b4 = AveragePooling2D(pool_size=(int(np.ceil(input_shape[0] / OS)), int(np.ceil(input_shape[1] / OS))))(input_tensor)
        ...
        # simple 1x1
        b0 = Conv2D(256, (1, 1), padding='same', use_bias=False, name='aspp0')(input_tensor)
        ...
        if backbone == 'xception':
            # rate = 6 (12)
            b1 = SepConv_BN(input_tensor, 256, 'aspp1',
                            rate=atrous_rates[0], depth_activation=True, epsilon=1e-5)
            # rate = 12 (24)
            b2 = SepConv_BN(input_tensor, 256, 'aspp2',
                            rate=atrous_rates[1], depth_activation=True, epsilon=1e-5)
            # rate = 18 (36)
            b3 = SepConv_BN(input_tensor, 256, 'aspp3',
...
#    print((model.layers[157].name))
#    print(type(model.layers[157].get_weights()))
#    import sys
#    sys.exit()
    
#    print(model.layers[144].name)
#    weights_3= model.layers[144].get_weights()[0]
#    print(weights_3[:3])
...
if __name__ == "__main__":
    model = Deeplabv3p(weights='pascal_voc', input_tensor=None, infer = False,
              input_shape=(512, 512, 3), classes=2, backbone='mobilenetv2',
              OS=16, alpha=1.,include_top = False)
#    model.summary()
